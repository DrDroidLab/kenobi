...
from protos.event.engine_options_pb2 import ColumnOption
...
class Column(BaseColumn, Options):
    ...
    is_groupable: bool
    is_orderable: bool
    aggregation_functions: Optional[list] = field(default_factory=list, repr=False)
    token_filter_op: Optional[object] = None
    id_options_cb: Optional[Callable] = None
    supported_ops: Optional[list] = field(default_factory=list, repr=False)
    id_label_resolver_cb: Optional[Callable] = None

    # TODO: Enable id literal resolution
    ...
    def get_options(self, account, obj):
        ...
        if self.type == LiteralType.ID:
            id_option = ColumnOption.IdOption(
                ...
                long_options=self.id_options_cb(account, obj)
            )
        return ColumnOption(
            name=self.name,
            id_option=id_option,
